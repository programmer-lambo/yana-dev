...
<head>
    ...
    <title>Notes Kategori</title>
    @vite(['resources/css/app.css', 'resources/js/app.js'])
</head>
<body class="bg-gray-100 min-h-screen">

<div x-data="categoryNotes()" x-init="init()">
    ...
    <main class="max-w-5xl mx-auto py-8 px-4">
        ...
        <div class="flex items-center gap-3 mb-6">
            <a href="/categories" class="text-sm text-gray-400 hover:text-gray-600">← Kategori</a>
            <span class="text-gray-300">|</span>
            <h2 class="text-2xl font-bold" x-text="categoryName ? 'Notes: ' + categoryName : 'Memuat...'"></h2>
        </div>
    ...
    function categoryNotes() {
        return {
            ...
            async init() {
                ...
                // Ambil category ID dari URL: /categories/1/notes
                const parts = window.location.pathname.split('/');
                this.categoryId = parts[2];
            ...
            async fetchNotes(page = 1) {
                ...
                try {
                    const res = await fetch(`/api/categories/${this.categoryId}/notes?page=${page}`, {
                        headers: {
                    ...
                    // Ambil nama kategori dari note pertama
                    if (this.notes.length > 0) {
                        this.categoryName = this.notes[0].category?.name;
                    }
