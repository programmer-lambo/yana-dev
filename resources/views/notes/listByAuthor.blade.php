                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <h2 class="text-2xl font-bold mt-1"
                            x-text="authorName ? 'Notes oleh ' + authorName : 'Memuat...'"></h2>
                        <p class="text-sm text-gray-400 mt-1">
                            <span x-text="followersCount"></span> followers
                            <span x-show="meta"> • <span x-text="meta?.total_data"></span> notes</span>
                        </p>
                    </div>

                    <!-- Tombol Follow -->
                    <button @click="toggleFollow" :disabled="followLoading"
                        class="px-5 py-2 rounded-lg text-sm font-semibold transition" :class="isFollowing
                ? 'bg-gray-100 hover:bg-red-50 text-gray-700 hover:text-red-500 border border-gray-300 hover:border-red-300'
                : 'bg-blue-600 hover:bg-blue-700 text-white'">
                        <span x-show="followLoading">...</span>
                        <span x-show="!followLoading && isFollowing">✓ Mengikuti</span>
                        <span x-show="!followLoading && !isFollowing">+ Ikuti</span>
                    </button>
                </div>
            </div>
            <!-- Loading Skeleton -->
                ...
                <div>
                        ...
                        <template x-for="note in notes" :key="note.id">
                            <div class="bg-white rounded-xl p-5 shadow hover:shadow-md transition">
                                ...
                                <div class="flex justify-between items-center mt-4">
                                    ...
                                    <a :href="note.slug ? `/notes/${note.slug}` : `#`"
                    ...
                    <template x-if="meta && meta.last_page > 1">
                        <div class="flex justify-center items-center gap-2 mt-8">
                            ...
                            <button @click="fetchNotes(meta.current_page + 1)"
        ...
        function authorNotes() {
            return {
                ...
                async fetchStats() {
                    const token = localStorage.getItem('token');
                    try {
                        const res = await fetch(`/api/authors/${this.authorId}/stats`, {
                            headers: {
                        ...
                        this.isFollowing = data.data.auth_user_status.is_following;
                        this.followersCount = data.data.stats.followers_count;

                        if (data.data.author_name) {
                            this.authorName = data.data.author_name;
                        }
                    } catch (err) {
                ...
                async toggleFollow() {
                    ...
                    try {
                        const res = await fetch(`/api/authors/${this.authorId}/follows`, {
                            method: 'GET',
                            headers: {
                                'Accept': 'application/json',
                                'Authorization': 'Bearer ' + token,
