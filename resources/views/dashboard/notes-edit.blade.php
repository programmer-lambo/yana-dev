        ...
        <main class="max-w-3xl mx-auto py-8 px-4">
            ...
            <div class="bg-white rounded-xl p-6 shadow space-y-5">
                ...
                <div>
                    ...
                    <input type="text" x-model="form.title" placeholder="Judul note..."
                ...
                <div>
                    ...
                    <textarea x-model="form.body" rows="10" placeholder="Tulis isi note di sini..."
                        class="w-full border rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 resize-y"
                        :class="errors.body ? 'border-red-400' : 'border-gray-300'"></textarea>
                ...
                <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                    ...
                    <button type="button" @click="form.is_indexed = !form.is_indexed"
                        class="relative inline-flex h-6 w-11 items-center rounded-full transition"
                        :class="form.is_indexed ? 'bg-blue-600' : 'bg-gray-300'">
                        <span class="inline-block h-4 w-4 transform rounded-full bg-white transition"
                            :class="form.is_indexed ? 'translate-x-6' : 'translate-x-1'"></span>
                    </button>
                </div>
        ...
        function noteForm() {
            return {
                ...
                async fetchNote() {
                    const token = localStorage.getItem('token');
                    try {
                        const res = await fetch(`/api/notes/${this.slug}`, {
                            headers: {
                                'Accept': 'application/json',
                                'Authorization': 'Bearer ' + token,
                            }
                        ...
                        this.form.category_id = note.category?.id ?? '';
                        this.form.is_indexed = !!note.is_indexed;

                    } catch (err) {
                        this.error = 'Gagal memuat data note.';
                    }
                },
